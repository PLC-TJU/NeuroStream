classdef StimulateClientApp < handle
    %
    % Usage:
    %     app.StimulateClientApp
    ...
    properties (Constant)
        AppPathBase = {'/StimulateClient'};
        AppClass = 'StimulateClientApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = StimulateClientApp()
        ...
        function startApp(obj)
            ...
            mlock;
            StimulateClientApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'StimulusProgram')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong StimulusProgram
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'StimulusProgram').getString, ...
            ...
            if nargout(@StimulusProgram) == 0
                eval('StimulusProgram');
            else
                obj.AppHandle = eval('StimulusProgram');
            end
