...
%%
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA] = plc_newtimef( data, frames, tlimits, Fs, varwin, varargin);
...
switch lower(g.plotersp)
    case 'on'
        ...
        if ~strcmpi(g.freqscale, 'log')
            imagesc(times/1000,freqs,PP(:,:), g.erspmax);
        else
            imagesclogy(times/1000,freqs,PP(:,:),g.erspmax);
        end;
        ...
        hold on
        plot([0 0],[0 freqs(end)],'--r','LineWidth',g.linewidth); % plot time 0
...
%        %% Panlincong 2021.3.16 注释（取消左边和底部的作图）
