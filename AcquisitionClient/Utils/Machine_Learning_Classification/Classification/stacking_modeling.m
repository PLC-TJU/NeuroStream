...
% Data: 2025.6.2
function model = stacking_modeling(data, label, algs, fs, times, freqs, chans, varargin)
% STACKING_TRAIN 训练一个基于多时间窗、多频带、多通道和多算法的Stacking集成模型
% 使用并行计算加速子模型训练

% 输入:
%   data: EEG数据 (通道×时间点×样本数)
%   label: 样本标签 (样本数×1)
...
if useParallel
    ...
    parfor i = 1:nSubModels
        ...
        % 数据预处理：时频滤波和通道选择
        fdata = ERPs_Filter(data, config.freq_band, config.chan_idx, config.time_win, fs);
        ...
        % 训练子模型
        subModel = p_modeling(fdata, label, config.alg, varargin{:});%#ok
        subModel.Wrsf = Wrsf;
        
        % 在训练集上测试子模型（得到预测标签和决策值）
        [pred, dv, ~] = p_classify(subModel, fdata, label);
    ...
    for i = 1:nSubModels
        ...
        % 训练子模型
        subModel = p_modeling(fdata, label, config.alg, varargin{:});
        subModel.Wrsf = Wrsf;
...
model.metaModel = train_classifier(metaFeatures, label, model.classifierType, ...
