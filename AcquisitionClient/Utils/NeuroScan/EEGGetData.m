%LC.Pan 2025.6.1 (tcpip)
function [DataBuf, tempData] = EEGGetData(TCPHandle, DataBuf)
    % 参数配置
    ...
    for i = 1:packetCount
        % 读取包头
        fread(TCPHandle, headerSize);
        
        % 读取完整数据包体（保持原始字节流）
        rawPacket = fread(TCPHandle, Channum*pointsPerPacket*bytesPerChannel, 'uint8');
